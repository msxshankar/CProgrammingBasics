...
int gridsize, winlength;
int newGameCheck;
int i, j;
int gridLimit, winLimit;
int row, col;
char symbol = 'O';
int repeatCondition, playerwon;
int full_check;
...
void showGrid ()
{
	printf("\n");
	printf("\t0 1 2\n\n");
	printf("0\t""%c"" ""%c"" ""%c", grid[0][0], grid[0][1], grid[0][2]);
	printf("\n");
	printf("1\t""%c"" ""%c"" ""%c", grid[1][0], grid[1][1], grid[1][2]);
	printf("\n");
	printf("2\t""%c"" ""%c"" ""%c", grid[2][0], grid[2][1], grid[2][2]);
	printf("\n");
	printf("\n");
...
int makeMove(int row, int col, char symbol)
{
	...
	if ((row < 0) || (col < 0) || (row > gridLimit-1) || (col > gridLimit -1)) {
		showErrIndex();
		return 0;
	}
...
{
	int hori_count = 0, vert_count = 0, hori_found = 0, found_count = 0;
	while (hori_found == 0) {
		if (hori_count == gridLimit) {
			if (vert_count == gridLimit) {
				return 0;
			}
			else {
				vert_count++;
		...
		if (grid[vert_count][hori_count] == symbol) {
			for (i = hori_count+1; i < length; i++) {
				if (grid[vert_count][i] == symbol) {
					found_count++;
				}
				else {
					hori_count++;
...
int checkVertical (char symbol, int length)
{
	...
	while (hori_found == 0) {
		...
		if (grid[vert_count][hori_count] == symbol) {
			for (i = vert_count+1; i < length; i++) {
				if (grid[i][hori_count] == symbol) {
					found_count++;
				}
				else {
					hori_count++;
...
{
	int hori_count = 0, vert_count = 0, hori_found = 0, found_count = 0, vert_iterate = 0, hori_iterate = 0;
	while (hori_found == 0) {
		if (hori_count == gridLimit-1) {
			if (vert_count == gridLimit-1) {
				return 0;
			}
			else {
				vert_count++;
		...
		if (grid[vert_count][hori_count] == symbol) {
			if ((vert_count == gridLimit - 1) || (hori_count == gridLimit - 1)) {
				hori_count++;
				continue;
			}
			vert_iterate = vert_count + 1, hori_iterate = hori_count + 1;
			for (i = 1; i < length; i++) {
				if (grid[vert_iterate][hori_iterate] == symbol) {
					found_count++;
					vert_iterate++, hori_iterate++;
				}
				else {
					hori_count++;
					break;
...
int checkAntiDiagonals (char symbol, int length)
{
	...
	while (hori_found == 0) {
		...
		if (grid[vert_count][hori_count] == symbol) {
			if ((vert_count == gridLimit - 1) || (hori_count == 0)) {
				hori_count++;
				continue;
			}
			vert_iterate = vert_count + 1, hori_iterate = hori_count - 1;
			for (i = 1; i < length; i++) {
				if (grid[vert_iterate][hori_iterate] == symbol) {
					found_count++;
					vert_iterate = vert_iterate + 1;
					hori_iterate = hori_iterate - 1;
				}
				else {
					hori_count++;
					break;
...
{
	int player_won_horizontal = 0, player_won_vertical = 0, player_won_diagonal = 0, player_won_antidiagonal = 0;
	player_won_horizontal = checkHorizontal(symbol, winLimit);
	player_won_vertical = checkVertical(symbol, winLimit);
	player_won_diagonal = checkDiagonals(symbol, winLimit);
	player_won_antidiagonal = checkAntiDiagonals(symbol, winLimit);
	if ((player_won_horizontal || player_won_vertical || player_won_diagonal || player_won_antidiagonal) == 1) {
		showWinMessage(symbol);
		repeatCondition = 0;
		return 1;
	}
	else if ((player_won_horizontal || player_won_vertical || player_won_diagonal || player_won_antidiagonal) == -1) 
		return -1;
	else {
		return 0;
	}
...
int  main (int argc, char* argv[])
{
	...
	newGameCheck = newGame(gridsize, winlength);
	while (newGameCheck == 0) {
		newGameCheck = newGame(gridsize, winlength);
	}
	initGrid();
	showGrid();
	repeatCondition = 1;
	while (repeatCondition == 1) {
		changeSymbol();
		makeMove(row, col, symbol);
		showGrid();
		full_check = boardIsFull();
			if (full_check == 1)
				repeatCondition = 0;
		playerHasWon(symbol, winlength);
	}
	askReplay();
